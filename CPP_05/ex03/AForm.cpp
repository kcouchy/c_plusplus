...
/*   Updated: 2024/07/23 12:09:24 by kcouchma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
AForm::~AForm (void)
{
	std::cout
		...
		<< this->getName() << " form has been shredded"
		<< std::endl;
	return ;
}
...
void			AForm::execute(Bureaucrat const & executor) const
{
	if (getIsSigned() == false)
	{
		std::cout
			<< executor.getName() 
			<< " cannot execute the " << this->getName() 
			<< " form because it is not signed"
			<< std::endl;
		return;
	}
	if	(executor.getGrade() > getExecLevel())
	{
		std::cout
			<< executor.getName() 
			<< " cannot execute the " << this->getName() 
			<< " form because their grade is too low"
			<< std::endl;
		throw GradeTooLowException();
		return;
	}
